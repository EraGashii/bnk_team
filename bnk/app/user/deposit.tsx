...
const Deposits = () => {
  ...
  const userId = 1; // For demo purposes, replace with actual user ID or get from auth context

  // Fetch deposits for the user
  useEffect(() => {
    setLoading(true);
    fetch(`http://localhost:4000/api/deposits/user/${userId}`)
  ...
  const createDeposit = async (e: React.FormEvent) => {
    ...
    const newDeposit = { userId, amount, method, date };
