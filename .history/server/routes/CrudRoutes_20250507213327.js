...
router.get("/Planet", async (_, res) => {
  try {
    const Planets = await Team.findAll();
    res.json(Teams);
  } catch (error) {
...
router.delete("/Team/:id", async (req, res) => {
  ...
  try {
    await Satelite.destroy({ where: { TeamId:id } });
...
router.get("/Satelite", async (_, res) => {
  try {
    const Satelites = await Satelite.findAll({ include:Team });
    res.json(Satelites);
  } catch (error) {
    handleError(res, error);
  }
...
router.post("/Satelite", async (req, res) => {
  ...
  try {
    const newSatelite = await Satelite.create({ Name, Number, BirthYear,TeamId });
