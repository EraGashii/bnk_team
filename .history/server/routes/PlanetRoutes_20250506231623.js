...
router.post("/Satelite", async (req, res) => {
  ...
  try {
    const newSatelite = await Satelite.create({ Title,Description,PlanetId });
...
router.put("/Satelite/:id", async (req, res) => {
  ...
  try {
    await Satelite.update({ Title, Description, PlanetId }, { where: { Id: id } });
...
router.delete("/Satelite/:id", async (req, res) => {
  try {
    await Satelite.destroy({ where: { id: req.params.id } });
