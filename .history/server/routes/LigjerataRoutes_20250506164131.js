...
router.get("/Ligjeruesis", async (req, res) => {
  ...
  router.post("/Ligjeruesis", async (req, res) => {
    ...
    try {
      ...
      console.error("❌ Error in POST /Ligjeruesis:", error);
      res.status(500).json({ error: error.message || "Internal Server Error" });
    }
  });
...
router.post("/Ligjeratas", async (req, res) => {
  ...
  try {
    const newLecture = await Ligjerata.create({ LectureName, LecturerId }); // ✅ FIXED: use new variable name
    res.json({ message: "Ligjerata added", Ligjerata: newLecture });       // ✅ safe
  } catch (error) {
    handleError(res, error);
  }
});
...
router.put("/Ligjeratas/:id", async (req, res) => {
  ...
  const {LectureName, LecturerId } = req.body;
  try {
    const [updated] = await Ligjerata.update({ LectureName, LecturerId }, { where: { id } });
    if (!updated) return res.status(404).json({ error: "Ligjerata not found" });
