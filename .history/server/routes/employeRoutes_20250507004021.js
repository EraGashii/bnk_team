...
router.post("/Contract", async (req, res) => {
  const { Title, DescriptionEmployeeId } = req.body;

  if (!Title || !EmployeeId || !Description) {
    return res.status(400).json({ error: "All fields are required" });
  }

  try {
    const newContract = await Contract.create({Title, Description,EmployeeId });
...
router.put("/Contract/:id", async (req, res) => {
  ...
  try {
    await Contract.update({ Title, Description,EmployeeId }, { where: { id } });
