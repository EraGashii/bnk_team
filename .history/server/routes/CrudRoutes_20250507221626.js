...
router.put("/Planet/:id", async (req, res) => {
  ...
  const { Type} = req.body;
  if ( !Type ) return res.status(400).json({ error: "Planet name is required" });

  try {
    const [updated] = await Planet.update({ Type}, { where: {  Name: name } });
...
router.get("/Satelite/byPlanetName/:name", async (req, res) => {
  const { name } = req.params;

  try {
    const planet = await Planet.findOne({ where: { Name: name } });
    if (!planet) return res.status(404).json({ error: "Planet not found" });

    const satelites = await Satelite.findAll({
      where: { PlanetId: planet.PlanetId, isDeleted: false },
      include: Planet
    });
