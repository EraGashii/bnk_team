...
router.post("/Planet", async (req, res) => {
  const {Name } = req.body;
  if (!Name || !) return res.status(400).json({ error: "Planet name is required" });
  try {
...
router.put("/Planet/:id", async (req, res) => {
  ...
  try {
    const [updated] = await Planet.update({ Name}, { where: { PlanetId:id } });
...
router.post("/Satelite", async (req, res) => {
  ...
  if (!Name || !PlanetId || !Number || !BirthYear) {
    return res.status(400).json({ error: "All fields are required" });
  }
  try {
    const newSatelite = await Satelite.create({ Name, Number, BirthYear,PlanetId });
...
router.put("/Satelite/:id", async (req, res) => {
  ...
  try {
    await Satelite.update({  Name,  Number, BirthYear,PlanetId }, { where: {Id:id } });
