...
router.get("/teams", async (_, res) => {
  try {
    const teams = await Ligjeuesi.findAll();
    res.json(teams);
  } catch (error) {
...
router.post("/teams", async (req, res) => {
  ...
  try {
    const team = await Ligjeuesi.create({ Name });
    res.json({ message: "Ligjeuesi added", team });
  } catch (error) {
    handleError(res, error);
  }
});
...
router.delete("/teams/:id", async (req, res) => {
  ...
  try {
    ...
    await Ligjerata.destroy({ where: { LigjeuesiId: teamId } });

    const deleted = await Ligjeuesi.destroy({ where: { LigjeuesiId: teamId } });
