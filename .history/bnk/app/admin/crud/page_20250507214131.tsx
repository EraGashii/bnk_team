...
const PlanetSateliteeeManager: React.FC = () => {
  const [tab, setTab] = useState<"Sateliteee" | "Planets">("Sateliteee");
  const [Planets, setPlanets] = useState<Planet[]>([]);
  const [Sateliteee, setSatelitee] = useState<Satelitee[]>([]);
  const [SateliteeData, setSateliteeData] = useState({  Name:"",  Number:"", BirthYear:"", PlanetId: "" });
  ...
  const loadSatelitee = async () => {
    const data = await fetchSatelitee();
    setSatelitee(data);
  };

  const handleSateliteeInput = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    setSateliteeData({ ...SateliteeData, [e.target.name]: e.target.value });
  };

  const handleAddSatelitee = async () => {
    const fixedData = {
        ...SateliteeData,
        PlanetId:Number(SateliteeData.PlanetId), // convert to number
      };
    await addSatelitee(fixedData);
    setSateliteeData({ Name:"",  Number:"", BirthYear:"", PlanetId: "" });
    loadSatelitee();
  };
  

  const handleDeleteSatelitee = async(id:number) => {
   await deleteSatelitee(id);
    loadSatelitee();
  };
  ...
  return (
    <div className="flex min-h-screen bg-background text-white">
      ...
      <main className="flex-1 p-10">
        <h1 className="text-3xl font-bold mb-6">Planet & Satelitee Management</h1>
        <div className="flex space-x-4 mb-6">
          <button className={`px-4 py-2 rounded ${tab === "Satelitee" ? "bg-green-600 text-white" : "bg-gray-700"}`} onClick={() => setTab("Satelitee")}>Satelitee</button>
          <button className={`px-4 py-2 rounded ${tab === "Planets" ? "bg-green-600 text-white" : "bg-gray-700"}`} onClick={() => setTab("Planets")}>Planets</button>
        </div>

        {tab === "Satelitee" && (
          <div>
            <h2 className="text-xl font-semibold mb-4">Satelitee Management</h2>
            <div className="flex gap-2 mb-4">
              <input type="text" name="Name" placeholder="Name" value={SateliteeData.Name} onChange={handleSateliteeInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4" />   
              <input type="number" name="Number" placeholder="Number" value={SateliteeData.Number} onChange={handleSateliteeInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4" />   
              <input type="number" name="BirthYear" placeholder="BirthYear" value={SateliteeData.BirthYear} onChange={handleSateliteeInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4" />   
              <select name="PlanetId" value={SateliteeData.PlanetId} onChange={handleSateliteeInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4">
                <option value="">Select Planet</option>
                {Planets.map((t) => <option key={t.PlanetId} value={t.PlanetId}>{t.Name}</option>)}
              </select>
              <button onClick={handleAddSatelitee} className="bg-green-500 text-white px-4 rounded">+ Add</button>
            </div>
                ...
                {Satelitee.map((p, i) => (
                  <tr key={p.Id} className="border-t border-gray-700">
                    ...
                    <td className="p-2 text-center">
                      <button className="text-red-500 hover:text-red-600" onClick={() => handleDeleteSatelitee(p.Id)}>
                        <FiTrash2 size={18} />
                      </button>
                    </td>
                  </tr>
...
export default PlanetSateliteeManager;
