...
const PlanetContractManager: React.FC = () => {
  const [tab, setTab] = useState<"Contract" | "Planets">("Contract");
  const [Planets, setPlanets] = useState<Planet[]>([]);
  const [Contract, setContract] = useState<Contract[]>([]);
  const [ContractData, setContractData] = useState({  Name:"", PlanetId: "" });
  ...
  const handleAddContract = async () => {
    const fixedData = {
        ...ContractData,
        PlanetId:Number(ContractData.PlanetId), // convert to number
      };
    await addContract(fixedData);
    setContractData({ Name:"", PlanetId: "" });
    loadContract();
  };
  ...
  return (
    <div className="flex min-h-screen bg-background text-white">
      ...
      <main className="flex-1 p-10">
        <h1 className="text-3xl font-bold mb-6">Planet & Contract Management</h1>
        ...
        {tab === "Contract" && (
          <div>
            ...
            <div className="flex gap-2 mb-4">
              ...
              <select name="PlanetId" value={ContractData.PlanetId} onChange={handleContractInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4">
            ...
            <table className="min-w-full border border-gray-700">
              <thead className="bg-gray-800 text-white">
                <tr>
                  ...
                  <th className="p-2 text-left">PlanetType</th>
                  <th className="p-2 text-center">Actions</th>
                </tr>
              </thead>
              <tbody className="bg-[#111]">
                {Planets.map((Planet,i) => (
                  <tr key={Planet.PlanetId} className="border-t border-gray-700">
                    ...
                    <td className="p-2 text-center space-x-3">
                      <button className="text-yellow-500 hover:text-yellow-600" onClick={() => { setEditPlanet(Planet);setName(Planet.Name);
    setType(Planet.Type);}}>
                        <FiEdit size={18} />
                      </button>
...
export default PlanetContractManager;
