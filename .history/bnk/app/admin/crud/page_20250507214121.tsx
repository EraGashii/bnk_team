...
const PlanetSateliteManager: React.FC = () => {
  ...
  const [Satelite, setSatelit] = useState<Satelit[]>([]);
    ...
    <div className="flex min-h-screen bg-background text-white">
      ...
      <main className="flex-1 p-10">
        ...
        <div className="flex space-x-4 mb-6">
          <button className={`px-4 py-2 rounded ${tab === "Satelit" ? "bg-green-600 text-white" : "bg-gray-700"}`} onClick={() => setTab("Satelit")}>Satelit</button>
          <button className={`px-4 py-2 rounded ${tab === "Planets" ? "bg-green-600 text-white" : "bg-gray-700"}`} onClick={() => setTab("Planets")}>Planets</button>
        </div>

        {tab === "Satelit" && (
          <div>
            <h2 className="text-xl font-semibold mb-4">Satelit Management</h2>
            <div className="flex gap-2 mb-4">
              <input type="text" name="Name" placeholder="Name" value={SatelitData.Name} onChange={handleSatelitInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4" />   
              <input type="number" name="Number" placeholder="Number" value={SatelitData.Number} onChange={handleSatelitInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4" />   
              <input type="number" name="BirthYear" placeholder="BirthYear" value={SatelitData.BirthYear} onChange={handleSatelitInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4" />   
              <select name="PlanetId" value={SatelitData.PlanetId} onChange={handleSatelitInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4">
                <option value="">Select Planet</option>
                {Planets.map((t) => <option key={t.PlanetId} value={t.PlanetId}>{t.Name}</option>)}
              </select>
              <button onClick={handleAddSatelit} className="bg-green-500 text-white px-4 rounded">+ Add</button>
            </div>
                ...
                {Satelit.map((p, i) => (
                  <tr key={p.Id} className="border-t border-gray-700">
                    ...
                    <td className="p-2 text-center">
                      <button className="text-red-500 hover:text-red-600" onClick={() => handleDeleteSatelit(p.Id)}>
                        <FiTrash2 size={18} />
                      </button>
                    </td>
                  </tr>
