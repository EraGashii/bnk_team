...
const PlanetPlayerManager: React.FC = () => {
  const [tab, setTab] = useState<"Player" | "Planets">("Player");
  const [Planets, setPlanets] = useState<Planet[]>([]);
  const [Player, setPlayer] = useState<Player[]>([]);
  const [PlayerData, setPlayerData] = useState({  Name:"",  Number:"", BirthYear:"", PlanetId: "" });
  ...
  const handleAddPlayer = async () => {
    const fixedData = {
        ...PlayerData,
        PlanetId:Number(PlayerData.PlanetId), // convert to number
      };
    await addPlayer(fixedData);
    setPlayerData({ Name:"",  Number:"", BirthYear:"", PlanetId: "" });
    loadPlayer();
  };
  ...
  return (
    <div className="flex min-h-screen bg-background text-white">
      ...
      <main className="flex-1 p-10">
        <h1 className="text-3xl font-bold mb-6">Planet & Player Management</h1>
                ...
                {Planets.map((Planet,i) => (
                  <tr key={Planet.PlanetId} className="border-t border-gray-700">
                    ...
                    <td className="p-2 text-center space-x-3">
                      ...
                      <button className="text-red-500 hover:text-red-600" onClick={() => handleDeletePlanet(Team.TeamId)}>
                        <FiTrash2 size={18} />
                      </button>
                    </td>
                  </tr>
