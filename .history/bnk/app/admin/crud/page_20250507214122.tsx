...
const PlanetSateliteeManager: React.FC = () => {
  ...
  const [Satelitee, setSatelite] = useState<Satelite[]>([]);
  const [SateliteData, setSateliteData] = useState({  Name:"",  Number:"", BirthYear:"", PlanetId: "" });
  ...
  const handleAddSatelite = async () => {
    const fixedData = {
        ...SateliteData,
        PlanetId:Number(SateliteData.PlanetId), // convert to number
      };
    await addSatelite(fixedData);
    setSateliteData({ Name:"",  Number:"", BirthYear:"", PlanetId: "" });
    loadSatelite();
  };
        ...
        {tab === "Satelite" && (
          <div>
            ...
            <div className="flex gap-2 mb-4">
              <input type="text" name="Name" placeholder="Name" value={SateliteData.Name} onChange={handleSateliteInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4" />   
              <input type="number" name="Number" placeholder="Number" value={SateliteData.Number} onChange={handleSateliteInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4" />   
              <input type="number" name="BirthYear" placeholder="BirthYear" value={SateliteData.BirthYear} onChange={handleSateliteInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4" />   
              <select name="PlanetId" value={SateliteData.PlanetId} onChange={handleSateliteInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4">
