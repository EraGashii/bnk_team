...
const EmployeePlayerManager: React.FC = () => {
  const [tab, setTab] = useState<"Player" | "Employees">("Player");
  const [Employees, setEmployees] = useState<Employee[]>([]);
  const [Player, setPlayer] = useState<Player[]>([]);
  const [PlayerData, setPlayerData] = useState({  Title:"", Description:"", EmployeeId: "" });
  ...
  const handleAddPlayer = async () => {
    const fixedData = {
        ...PlayerData,
        EmployeeId:Number(PlayerData.EmployeeId), // convert to number
      };
    await addPlayer(fixedData);
    setPlayerData({ Title:"", Description:"",EmployeeId: "" });
    loadPlayer();
  };
  ...
  const handleAddEmployee = async () => {
    ...
    await addEmployee( Name,Surname);
    setName("");  
    setSurname("");
    loadEmployees();
  };
  ...
  const handleUpdateEmployee = async () => {
    if (editEmployee) {
      await updateEmployee(editEmployee.EmployeeId,Name,Surname);
  ...
  return (
    <div className="flex min-h-screen bg-background text-white">
      ...
      <main className="flex-1 p-10">
        <h1 className="text-3xl font-bold mb-6">Employee & Player Management</h1>
        ...
        {tab === "Player" && (
          <div>
            ...
            <div className="flex gap-2 mb-4">
              <input type="text" name="Title" placeholder="Title" value={PlayerData.Title} onChange={handlePlayerInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4" />   
              <input type="text" name="Description" placeholder="Description" value={PlayerData.Description} onChange={handlePlayerInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4" />   
              <select name="EmployeeId" value={PlayerData.EmployeeId} onChange={handlePlayerInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4">
              ...
              <tbody className="bg-[#111]">
                {Player.map((p, i) => (
                  <tr key={p.Id} className="border-t border-gray-700">
                    ...
                    <td className="p-2">{p.Employee?.Surname ||"N/A"}</td>
            ...
            <table className="min-w-full border border-gray-700">
              <thead className="bg-gray-800 text-white">
                <tr>
                  ...
                  <th className="p-2 text-left">EmployeeSurnamee</th>
                  <th className="p-2 text-center">Actions</th>
                </tr>
              </thead>
              <tbody className="bg-[#111]">
                {Employees.map((Employee,i) => (
                  <tr key={Employee.EmployeeId} className="border-t border-gray-700">
                    ...
                    <td className="p-2 text-center space-x-3">
                      <button className="text-yellow-500 hover:text-yellow-600" onClick={() => { setEditEmployee(Employee);setName(Employee.Name);
...
export default EmployeePlayerManager;
