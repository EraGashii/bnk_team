...
const PlanetSateliteManager: React.FC = () => {
  ...
  const [Name, setLecturerName] = useState<string>("");
  ...
  const handleAddPlanet = async () => {
    if (!LecturerName.trim()) return alert("Planet name is required");
    await addPlanet( LecturerName,department,email);
    setLecturerName("");
    setDepartment("");
    setEmail("");
    loadPlanets();
  ...
  const handleUpdatePlanet = async () => {
    if (editPlanet) {
      await updatePlanet(editPlanet.LecturerId,LecturerName,department,email);
      setEditPlanet(null);
      setLecturerName("");
    setDepartment("");
    setEmail("");
              ...
              <tbody className="bg-[#111]">
                {Satelite.map((p, i) => (
                  <tr key={p.Id} className="border-t border-gray-700">
                    ...
                    <td className="p-2">{p.Planet?.LecturerName ||"N/A"}</td>
