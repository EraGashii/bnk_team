...
const PlanetSateliteManager: React.FC = () => {
  ...
  const handleAddSatelite = async () => {
    ...
    setSateliteData({  LectureName: "", PlanetId: "" });
    loadSatelite();
  };
          ...
          <div>
            ...
            <div className="flex gap-2 mb-4">
              ...
              <select name="PlanetId" value={SateliteData. PlanetId} onChange={handleSateliteInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4">
                ...
                {Planets.map((t) => <option key={t.PlanetId} value={t.PlanetId}>{t.LecturerName}</option>)}
              </select>
              ...
              <tbody className="bg-[#111]">
                {Satelite.map((p, i) => (
                  <tr key={p.Id} className="border-t border-gray-700">
                    ...
                    <td className="p-2">{p.Planet?.Type  ||"N/A"}</td>
              ...
              <tbody className="bg-[#111]">
                {Planets.map((Planet,i) => (
                  <tr key={Planet.PlanetId} className="border-t border-gray-700">
                    ...
                    <td className="p-2">{Planet.Type}</td>

                    <td className="p-2 text-center space-x-3">
                      <button className="text-yellow-500 hover:text-yellow-600" onClick={() => { setEditPlanet(Planet); setName(Planet.Name);setType(Planet.Type);}}>
                        <FiEdit size={18} />
                      </button>
