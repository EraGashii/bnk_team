...
const LigjeruesiContractManager: React.FC = () => {
  const [tab, setTab] = useState<"Contract" | "Ligjeruesis">("Contract");
  const [Ligjeruesis, setLigjeruesis] = useState<Ligjeruesi[]>([]);
  const [Contract, setContract] = useState<Contract[]>([]);
  const [ContractData, setContractData] = useState({ Title: "",Description:"", LigjeruesiId: "" });
  ...
  const handleAddContract = async () => {
    const fixedData = {
      ...ContractData,
      LigjeruesiId: Number(ContractData.LigjeruesiId), // 🛠 CONVERT to number
    };
  
    await addLigjerata(fixedData);
    setContractData({ Title: "", Description: "", LigjeruesiId: "" });
    loadContract();
  };
  ...
  const handleUpdateLigjeruesi = async () => {
    if (editLigjeruesi) {
      await updateLigjeruesi(editLigjeruesi.LigjeruesiId, Name,Surname);
  ...
  return (
    <div className="flex min-h-screen bg-background text-white">
      ...
      <main className="flex-1 p-10">
        <h1 className="text-3xl font-bold mb-6">Ligjeruesi & Contract Management</h1>
        ...
        {tab === "Contract" && (
          <div>
            ...
            <div className="flex gap-2 mb-4">
              ...
              <select name="LigjeruesiId" value={ContractData.LigjeruesiId} onChange={handleContractInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4">
                <option value="">Select Ligjeruesi</option>
                {Ligjeruesis.map((t) => <option key={t.LigjeruesiId} value={t.LigjeruesiId}>{t.Name}</option>)}
              </select>
            ...
            <table className="min-w-full border border-gray-700">
              ...
              <tbody className="bg-[#111]">
                {Ligjeruesis.map((Ligjeruesi, i) => (
                  <tr key={Ligjeruesi.LigjeruesiId} className="border-t border-gray-700">
                    <td className="p-2">{i + 1}</td>
                    ...
                    <td className="p-2 text-center space-x-3">
                      ...
                      <button className="text-red-500 hover:text-red-600" onClick={() => handleDeleteLigjeruesi(Ligjeruesi.LigjeruesiId)}>
                        <FiTrash2 size={18} />
                      </button>
                    </td>
                  </tr>
...
export default LigjeruesiContractManager;
