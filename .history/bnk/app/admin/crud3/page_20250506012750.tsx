...
const LigjeruesiLigjerataManager: React.FC = () => {
  ...
  const handleUpdateLigjeruesi = async () => {
    if (editLigjeruesi) {
      await updateLigjeruesi(editLigjeruesi. LecturerId,department,email);
          ...
          <div>
            ...
            <div className="flex gap-2 mb-4">
              ...
              <select name=" LecturerId" value={LigjerataData. LecturerId} onChange={handleLigjerataInput} className="bg-[#222] text-white border border-gray-700 px-3 py-2 w-1/4">
                ...
                {Ligjeruesis.map((t) => <option key={t. LecturerId} value={t. LecturerId}>{t.LectureName}</option>)}
              </select>
                ...
                {Ligjeruesis.map((Ligjeruesi, i) => (
                  <tr key={Ligjeruesi. LecturerId} className="border-t border-gray-700">
                    ...
                    <td className="p-2 text-center space-x-3">
                      <button className="text-yellow-500 hover:text-yellow-600" onClick={() => { setEditLigjeruesi(Ligjeruesi); setLecturerName(Ligjeruesi.LecturerName);setDepartment(Ligjeruesi. Department);setDepartment(Ligjeruesi. Department);}}>
                        <FiEdit size={18} />
                      </button>
