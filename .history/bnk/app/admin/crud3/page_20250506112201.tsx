...
const LigjeruesiLigjerataManager: React.FC = () => {
  ...
  const handleAddLigjerata = async () => {
    const fixedData = {
      ...
      const handleAddLigjerata = async () => {
        if (!LigjerataData.LectureName.trim() || !LigjerataData.LecturerId) {
          return alert("All fields are required.");
        }
      
        const fixedData = {
          LectureName: LigjerataData.LectureName,
          LecturerId: Number(LigjerataData.LecturerId),
        };
        ...
        try {
          ...
          alert("Failed to add Ligjerata: " + (err?.response?.data?.error || err.message));
        }
      };
